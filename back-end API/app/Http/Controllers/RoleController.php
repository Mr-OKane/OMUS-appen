...
class RoleController extends Controller
{
    ...
    {
        Auth::user()->role->permissions->contains(Permission::firstWhere('name', '=', 'role.permission.viewAny'))
            ? Response::allow()
            : Response::deny('you are not the chosen one');
    ...
    public function update(UpdateRoleRequest $request, Role $role)
    {
        ...
        $object = Role::withTrashed()->where('id','=',$role->id)->first();
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function rolePermissionsUpdate(UpdateRoleRequest $request, Role $role)
    {
        ...
        if (Auth::user()->role->permissions->contains(Permission::firstWhere('name', '=', 'role.update'))){
            ...
            if (!empty($request['absence_create'])){
                array_push($permissionIDs,
                    Permission::firstWhere('name','=','absence.create')->id
                );
            }
